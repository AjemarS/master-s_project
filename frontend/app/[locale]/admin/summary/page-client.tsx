...
import type { Product, Order, RevenueReport, SalesReport } from "~/lib/types";
import { StatsCardSkeleton } from "../components";
import { useRecentProducts } from "~/lib/hooks/use-recent-products";
import { reportApi, orderApi, warehouseApi, stockApi } from "~/lib/api/admin-api";
import { PieChart as RechartPie, Pie, Cell, Tooltip, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid } from "recharts";
...
export default function SummaryPageClient({ initialProducts }: SummaryPageClientProps) {
  ...
  const [warehouseOccupancy, setWarehouseOccupancy] = useState<{ name: string; items: number; quantity: number }[]>([]);
  ...
  useEffect(() => {
    ...
    reportApi.dailySales().then((res) => {
      if (res.data?.daily) setDailyRevenue(res.data.daily);
    });
    reportApi.sales().then((res) => { if (res.data) setSalesData(res.data); });
    Promise.all([warehouseApi.getAll(), stockApi.getAll()]).then(([whRes, stRes]) => {
      if (whRes.data?.results && stRes.data?.results) {
        const occupancy = whRes.data.results.map((wh) => {
          const whStock = stRes.data!.results.filter((s) => s.warehouse_name === wh.name);
          return {
            name: wh.name,
            items: whStock.length,
            quantity: whStock.reduce((s, st) => s + st.quantity, 0),
          };
        });
        setWarehouseOccupancy(occupancy);
      }
    });
  }, []);
...
{tSum("lowStock")}
        ...
        {/* Channel pie + warehouse occupancy + quick actions */}
                  ...
                  {warehouseOccupancy.map((wh) => (
                    <div key={wh.name}>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-slate-500 dark:text-slate-400">{wh.quantity} од.</span>
                      </div>
            ...
            <CardContent className="space-y-3">
              {isAdmin && (
                <Button asChild variant="outline" className="w-full justify-start">
                  <Link href="/admin/products"><PlusCircle className="h-4 w-4 mr-2" /> Додати товар</Link>
                </Button>
              )}
              <Button asChild variant="outline" className="w-full justify-start">
                <Link href="/admin/orders"><ShoppingCart className="h-4 w-4 mr-2" /> Переглянути замовлення</Link>
              </Button>
              {(isAdmin || isWhWorker) && (
                <Button asChild variant="outline" className="w-full justify-start">
                  <Link href="/admin/goods-receipts"><FileText className="h-4 w-4 mr-2" /> Створити накладну</Link>
                </Button>
              )}
              {dailyRevenue.length > 0 && (
                <div className="mt-4 pt-4 border-t dark:border-slate-700">
                  <div className="text-xs text-slate-500 mb-2">Дохід за 30 днів</div>
