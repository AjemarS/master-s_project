...
export function TwoFactorPageClient() {
  ...
  const fetchBackupCodes = async () => {
    ...
    try {
      ...
      if ("data" in result && result.data && Array.isArray(result.data.backupCodes)) {
        setBackupCodes(result.data.backupCodes);
      } else {
        setBackupError("failed to retrieve backup codes. you may need to enable 2fa first.");
      }
    } catch {
      setBackupError("failed to retrieve backup codes. you may need to enable 2fa first.");
      ...
      <div className="w-full max-w-md space-y-8 rounded-lg border p-6 shadow-md">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">
            {isUsingBackupCode ? "Enter a backup code" : "Enter your authentication code"}
          </p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleVerify}>
          <div className="space-y-4">
            ...
            {!isUsingBackupCode && (
              <div className="flex items-start">
                ...
                <div className="ml-3 text-sm">
                  <label className="font-medium text-gray-700" htmlFor="trustDevice">
                    Trust this device
                  </label>
                  <p className="text-gray-500">
                    you won&apos;t need to enter a verification code on this device for 30 days
                  </p>
                </div>
              </div>
            )}
            ...
            {isUsingBackupCode && (
              <div className="mt-6">
                ...
                {backupLoading ? (
                  <div className="mb-4 text-center text-gray-500">loading backup codes...</div>
          ...
          <div className="flex flex-col space-y-4">
            ...
            <button
              ...
              type="button"
            >
              {isUsingBackupCode ? "Use authenticator app instead" : "Use a backup code instead"}
            </button>
          </div>
        </form>
      </div>
