...
// ── Handle event ────────────────────────────────────────────────
function handleEvent(routingKey, event, eventId) {
...
async function startConsumer() {
  ...
  channel.consume("notification.order.confirmed", (msg) => {
    const event = JSON.parse(msg.content.toString());
    handleEvent("order.created", event, event.event_id || msg.properties.messageId);
    channel.ack(msg);
  });

  channel.consume("notification.status_changed", (msg) => {
    const event = JSON.parse(msg.content.toString());
    handleEvent("order.status_changed", event, event.event_id || msg.properties.messageId);
    channel.ack(msg);
  });

  channel.consume("notification.order.cancelled", (msg) => {
    const event = JSON.parse(msg.content.toString());
    handleEvent("order.cancelled", event, event.event_id || msg.properties.messageId);
    channel.ack(msg);
  });

  channel.consume("notification.low_stock", (msg) => {
    const event = JSON.parse(msg.content.toString());
    handleEvent("inventory.low_stock", event, event.event_id || msg.properties.messageId);
    channel.ack(msg);
  });
...
// ── Express server (health + preferences) ───────────────────────
const app = express();
