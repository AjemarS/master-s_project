...
from orders.infrastructure.inventory_client import reserve_stock, deduct_stock, release_stock
...
def update_order_status(order, new_status, request=None):
    ...
    if new_status == Order.DELIVERING and order.warehouse_id:
        success = deduct_order_stock(order, request)
        if not success:
...
def deduct_order_stock(order, request=None):
    """Deduct stock for shipping. Returns True if all succeeded."""
    all_success = True
    for item in order.items.all():
